...
USAGE:
   ...
   python obspreproc_tcv.py --ncep_trkr_filename <The path to the file \
     containing the NCEP TC tracker output (fort.64)> --output_filename \
     <The path to the file to contain the output record(s)> \
     --tcv_filename <The path to the file containing the TC-vitals>
...
class ObsPreProcTCV(object):
    ...
    * opts_obj; a Python object containing the user command line
    ...
    def get_tcvid(self, ncep_trkr_str):
        ...
        This method constructs a TC event identifier string that,
        using the a NCEP tracker record, that is compliant with the
        TC-vitals formatted records; if one cannot be determined,
